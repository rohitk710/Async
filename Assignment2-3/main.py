# -*- generated by 1.0.12 -*-
import da
PatternExpr_230 = da.pat.TuplePattern([da.pat.ConstantPattern('request'), da.pat.FreePattern('c2'), da.pat.FreePattern('p')])
PatternExpr_321 = da.pat.TuplePattern([da.pat.ConstantPattern('ack'), da.pat.FreePattern('c2'), da.pat.BoundPattern('_BoundPattern325_')])
PatternExpr_421 = da.pat.TuplePattern([da.pat.ConstantPattern('release'), da.pat.FreePattern('c'), da.pat.FreePattern('p')])
_config_object = {}
import sys
controller = da.import_da('controller')

class P(controller.Controllee, da.DistProcess):

    def __init__(self, procimpl, props):
        super().__init__(procimpl, props)
        self._PReceivedEvent_1 = []
        self._events.extend([da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_0', PatternExpr_230, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_229]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_1', PatternExpr_321, sources=None, destinations=None, timestamps=None, record_history=True, handlers=[]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_2', PatternExpr_421, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_420])])

    def setup(self, ctl, s, nrequests, **rest_574):
        super().setup(ctl=ctl, s=s, nrequests=nrequests, **rest_574)
        self._state.ctl = ctl
        self._state.s = s
        self._state.nrequests = nrequests
        super().setup(self._state.ctl)
        self._state.q = set()
        self._state.timedOut = 0
        self._state.minimumTimeStamp = 100

    @controller.run
    def run(self):
        for i in range(self._state.nrequests):
            self.request()

    def request(self):
        super()._label('request', block=False)
        c = self.logical_clock()
        self.send(('request', c, self._id), to=self._state.s)
        self._state.q.add(('request', c, self._id))
        self.cs()

    def cs(self):
        super()._label('cs', block=False)
        self._state.minimumTimeStamp = 100
        for (tag, c, p2) in self._state.q:
            if ((tag == 'request') and (p2 == self._id) and (c < self._state.minimumTimeStamp)):
                self._state.minimumTimeStamp = c
        c = self._state.minimumTimeStamp
        super()._label('_st_label_286', block=False)
        p = c2 = None

        def UniversalOpExpr_288():
            nonlocal p, c2
            for (_ConstantPattern292_, c2, p) in self._state.q:
                if (_ConstantPattern292_ == 'request'):
                    if (not (((c2, p) == (c, self._id)) or ((c, self._id) < (c2, p)))):
                        return False
            return True
        p = c2 = None

        def UniversalOpExpr_314():
            nonlocal p, c2
            for p in self._state.s:

                def ExistentialOpExpr_319(p):
                    nonlocal c2
                    for (_, _, (_ConstantPattern336_, c2, _BoundPattern339_)) in self._PReceivedEvent_1:
                        if (_ConstantPattern336_ == 'ack'):
                            if (_BoundPattern339_ == p):
                                if (c2 > c):
                                    return True
                    return False
                if (not ExistentialOpExpr_319(p=p)):
                    return False
            return True
        _st_label_286 = 0
        self._timer_start()
        while (_st_label_286 == 0):
            _st_label_286 += 1
            if (UniversalOpExpr_288() and UniversalOpExpr_314()):
                self.output(((('In CS of process: ' + str(self._id)) + ' with timestamp : ') + str(c)))
                self.release()
                _st_label_286 += 1
            elif self._timer_expired:
                self._state.timedOut = (self._state.timedOut + 1)
                self.output(('Timed out for process: ' + str(self._id)))
                _st_label_286 += 1
            else:
                super()._label('_st_label_286', block=True, timeout=15)
                _st_label_286 -= 1

    def release(self):
        super()._label('release', block=False)
        self._state.minimumTimeStamp = 100
        for (tag, c, p2) in self._state.q:
            if ((tag == 'request') and (p2 == self._id) and (c < self._state.minimumTimeStamp)):
                self._state.minimumTimeStamp = c
        c = self._state.minimumTimeStamp
        self._state.q.remove(('request', c, self._id))
        self.send(('release', self.logical_clock(), self._id), to=self._state.s)

    def _P_handler_229(self, c2, p):
        self._state.q.add(('request', c2, p))
        self.send(('ack', self.logical_clock(), self._id), to=p)
    _P_handler_229._labels = None
    _P_handler_229._notlabels = None

    def _P_handler_420(self, c, p):
        self._state.minimumTimeStamp = 100
        for (tag, c, p2) in self._state.q:
            if ((tag == 'request') and (p2 == p) and (c < self._state.minimumTimeStamp)):
                self._state.minimumTimeStamp = c
        c = self._state.minimumTimeStamp
        for x in {('request', c, p) for (_ConstantPattern467_, c2, p2) in self._state.q if (_ConstantPattern467_ == 'request')}:
            self._state.q.remove(x)
            break
    _P_handler_420._labels = None
    _P_handler_420._notlabels = None

class Node_(da.NodeProcess):

    def __init__(self, procimpl, props):
        super().__init__(procimpl, props)
        self._events.extend([])
    _config_object = {'channel': 'fifo', 'clock': 'Lamport'}

    def run(self):
        nprocs = (int(sys.argv[1]) if (len(sys.argv) > 1) else 10)
        nrequests = (int(sys.argv[2]) if (len(sys.argv) > 2) else 10)
        ctl = self.new(controller.Controller, num=1)
        self._setup(ctl, (nprocs,))
        self._start(ctl)
        ps = self.new(P, num=nprocs)
        for p in ps:
            self._setup(p, (ctl, (ps - {p}), nrequests))
        self._start(ps)
